...
public class WebHandler
{
    ...
    private static void getDeckWithWebHandler() {
        ArrayList<SelectableDeck> arrayList = new ArrayList<SelectableDeck>();
        try
        {
            ...
            for (Element row : rows)
            {
                ...
                try
                {
                    selectableDeck.setDeckId(arrayList.size()+1);
                    selectableDeck.setDeckCoverUrl("src\\main\\resources\\com\\images\\DeckBacks\\deckBack_" + selectableDeck.getDeckId() + ".png");
                    selectableDeck.setDeckName(cells.get(1).text());
                    selectableDeck.setDeckDescription(cells.get(2).text());
                    selectableDeck.setUnlockCondition(cells.get(3).text());
                }
                catch (Exception e) {
                    System.out.println("Fehler aufgetreten!!");
                    System.out.println(e);
                }
                arrayList.add(selectableDeck);
            }
    ...
    private static void getSealWithWebHandler() {
        ...
        try
        {
            ...
            for (Element row : rows)
            {
                ...
                try
                {
                    seal.setSealID(arrayList.size()+1);
                    seal.setSealImageUrl("src\\main\\resources\\com\\images\\Stickers_Seals\\stamp_" + seal.getSealId() + ".png");
